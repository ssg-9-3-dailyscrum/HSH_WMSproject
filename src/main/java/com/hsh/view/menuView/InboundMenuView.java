...
public class InboundMenuView {
        ...
        while (!flag) {
            if ("총관리자".equals(role) || "창고관리자".equals(role)) {
                ...
                switch (choice) {
                    ...
                    case 2 -> viewInboundDetail(0, userId); // 0 -> 전체 조회
                ...
                switch (choice) {
                    ...
                    case 3 -> viewInboundDetail(userId, userId); // 본인 조회
    ...
    private void superAdminMenu() {
        ...
        System.out.println("3. 대기 입고 조회");
        System.out.println("4. 전체 입고 조회");
    ...
    private void memberMenu() {
        ...
        System.out.println("2. 입고 취소/수정");
        System.out.println("3. 입고 1건 조회");
        System.out.println("4. 대기 입고 조회");
        System.out.println("5. 돌아가기");
        System.out.println("=============================");
    }
    ...
    private void updateOrCancelInbound(int memberId) {
        try {
            System.out.print("수정/취소할 입고 ID 입력: ");
            int inboundId = Integer.parseInt(reader.readLine().trim());
            ...
            if (choice == 1) {
                ...
                System.out.println(success ? ":: 입고 수정 완료! ::" : ":: 입고 수정 실패 ::" );

            } else if (choice == 2) {
                boolean success = controller.cancelRequest(inboundId);
                System.out.println(success ? ":: 입고 취소 완료! ::" : ":: 입고 취소 실패 ::" );
            } else {
    ...
    private void viewInboundDetail(int inboundId, int userId) {
        InboundResponseDto dto = controller.getInboundDetail(inboundId, 1, userId);
        if (dto == null) {
            System.out.println(":: 조회할 입고가 없습니다. ::");
            return;
        }
